...
39. **overlay 详情层「添加/删除图纸」迁到底部左侧控制条**：用户截图希望把 overlay 的画廊操作按钮放在底部左侧。实现：
    ...
    - **`src/styles/editor.css`**：新增 `.overlay-edit-strip` 规则，默认 `display: none`；仅在 `html.detail-overlay-open body.edit-mode` 下以 `position: fixed; left: var(--pad-x); bottom: var(--pad-x); z-index: 9001` 显示为底左控制条，复用 `.edit-controls` 的黑白描边按钮样式。

...
38. **修复「在详情页改封面图，预览页项目图片不更新」**：用户报告在编辑页面改了项目封面图，但公开预览页项目图片没变（courtyard-house 封面 `...T11-48-22.jpg` → `...T19-05-39.jpg`，且 frontmatter 始终停留在旧图，说明压根没保存）。根因有两处，都指向「详情页是全站唯一带「换封面图」控件的编辑面，但保存与即时替换都没覆盖它」：
    - **保存侧**：`save()` 只收集 `.project-column .project-card, .info-overlay`；而「换封面图」`input[data-role="replace-image"]` 全站仅存在于项目详情页 `src/pages/projects/[slug].astro` 的 `<article class="project-detail project-card" data-path data-frontmatter>`。详情页没有列卡片，`persistCardChange()` 找不到可同步的列卡片，于是 `handleReplace` 标记的是详情 article 自己；`save()` 不收集它 → 发出的 `save-all` 里没有该 slug → 封面既不写回 GitHub frontmatter、也不写 `live/${slug}.json` 即时覆盖，预览永远显示旧图。修复：`save()` 选择器扩展为 `.project-column .project-card, .info-overlay, article.project-detail.project-card`；详情页无列卡片、/editor 无 `project-detail` article，二者不同时存在，dedup 按 path 不会冲突。
    - **即时替换侧（live-patch）**：公开详情页 hero 是 `article.project-detail.project-card`，它直接带 `data-slug`（内部没有 `a.view-link[data-slug]`）；而 `live-patch.ts` 原先用 `card.querySelector("a.view-link[data-slug]")` 取 slug，会误命中详情 article 内部 `.detail-related` 里「配对项目」`ProjectCard` 的**嵌套** `view-link`（slug 指向配对项目），导致覆盖查找 miss、hero 不替换。修复：对 `project-detail` 卡片直接读 `card.dataset.slug`（其自身 slug），普通卡片仍走内部 `a.view-link[data-slug]`。
    - **验证**：Playwright 实测——(A) 详情页把 courtyard-house 封面改为新 URL 并标记 dirty 后点「保存」，`save-all` 载荷含 `src/content/projects/courtyard-house.md` 且 `cover` 为新 URL；(B) mock `/api/live` 返回 `{courtyard-house:{cover_image:NEW}}` 后，公开详情页 hero 首图与首页对应 `.project-card` 的 `<img>` 均被替换为 NEW；(C) `/editor` 回归：列卡片保存仍正常、无 `project-detail` 路径泄漏进载荷。`pnpm build` 34 页通过、`check-pre-whitespace` 通过。红线：未改动高危文件（BaseLayout/index/global.css 未动）、global.css/ProjectCard.astro 未动、动画/Close/导航未动。

39. **overlay 详情层「添加/删除图纸」迁到底部左侧控制条**：用户截图希望把 overlay 的画廊操作按钮放在底部左侧。实现：
    - **`src/components/ProjectDetailOverlays.astro`**：`.edit-controls` 增加 `overlay-edit-strip` class，并新增 `<button type="button" data-role="delete-gallery">删除图纸</button>`（原来只有「添加图纸」）。
    - **`src/styles/editor.css`**：新增 `.overlay-edit-strip` 规则，默认 `display: none`；仅在 `html.detail-overlay-open body.edit-mode` 下以 `position: sticky; left: var(--pad-x); bottom: var(--pad-x); z-index: 100` 显示为底左控制条。相比 `position: fixed` 更贴合 overlay 的滚动容器：内容短时自然落在画廊底部，内容长时随滚动吸底，避免被裁出可视区而点不到。复用 `.edit-controls` 的黑白描边按钮样式。
    - **`src/scripts/edit-mode.ts`**：新增 `[data-role="delete-gallery"]` 点击处理，删除当前选中的 `.gallery-img-wrap.selected`（封面不删）。同时把原有逐张图 X 按钮的删除逻辑抽成 `removeGalleryWrap()` 并修复一个潜在 bug：必须先 `closest(".project-card")` 再 `wrap.remove()`，否则元素 detach 后找不到 owning card，会导致 frontmatter 不更新、dirty 不标记、删除的 R2 URL 也不记录。
    - **验证**：Playwright 8/8 通过——编辑态打开 overlay 后底左 strip 显示「添加图纸 + 删除图纸」；选中图后点「删除图纸」，DOM 移除、overlay 与列卡片均 dirty、frontmatter gallery 同步移除；底左「添加图纸」上传 mock 文件后 gallery 末尾新增 URL。`pnpm build` 34 页通过、`check-pre-whitespace` 通过。红线：未改动 `BaseLayout.astro` / `index.astro` / `global.css` / `ProjectCard.astro`，动画/Close/导航未动。

40. **修复 filter strip 点 × 删除 filter 后刷新又出现（第一轮，`filter-edit.js` 侧）**：用户在 `/editor/coupling/` 点 filter 词组旁的 × 删除，保存后刷新，被删 filter 又重新出现。根因：`filter-edit.js` 删除 filter 时会在 `localStorage` 写一条 `images: "__DELETED__"` 的 tombstone，但页面初始化时只在进入 filter 编辑态后才用 `mergedFilters()` 重建 strip；刷新后的初始静态 strip 直接由 `CouplingFilterStrip.astro` 服务端渲染，tombstone 未生效，导致在 Cloudflare 重建 / `live/filters.json` 生效前被删 filter 又显示出来。修复：在 `filter-edit.js` init 时立即应用 tombstone，把静态 strip 中对应 slug 的 `.filter-strip-item` 移除。这样即使 `live/filters.json` 尚未覆盖或用户提前刷新，已删 filter 也不会再现。验证：Playwright 模拟删除 `a a a`、mock `/api/save` 成功但 `/api/live` 不返回 filter 覆盖，刷新后 strip 只剩 6 个 filter；若 `/api/live` 返回新列表也能正确显示。`pnpm build` 34 页通过。红线：未改动高危文件。

41. **修复 filter 删除后刷新又出现（第二轮，`live-patch.ts` 兜底，覆盖条目 40 的残留）**：用户反馈条目 40 的修复「还是不行」。进一步定位：根因更深一层——`live-patch.ts` 由 `BaseLayout.astro` 在**所有页面**（含管理页 `/editor/coupling/`、`/editor/filter/[slug]/`）加载，它会在 `DOMContentLoaded` 时**清空整个 `.filter-strip` 并用 `/api/live` 的 `map.filters`（即 R2 上的 `live/filters.json`）重新注入** filter。而 `filter-edit.js` 的 init tombstone 删除先于 `live-patch.ts` 执行，于是只要 `live/filters.json` 仍含被删 slug（服务器保存失败 / R2 边缘缓存未及时更新 / Cloudflare 重建延迟），`live-patch.ts` 就把被删 filter 重新加回来。公开预览页 `/coupling/`、`/filter/[slug]/` 根本不加载 `filter-edit.js`，连条目 40 的 tombstone 修复都不存在，只能靠 `live/filters.json`——一旦陈旧就必现。修复：在 `live-patch.ts` 新增 `getDeletedFilterSlugs()`（读 `localStorage["am_filter_edit_v1"]`，收集 `images === "__DELETED__"` 的 slug），并在 filter 重注入循环里对被删 slug `continue` 跳过。这样删除在**所有页面、无论 `live/filters.json` 是否陈旧**都能跨刷新持久隐藏；服务器 `save-filters` 仍会把 `mergedFilters()`（已排除被删 slug）同时写入 `couplingFilters.ts` 与 `live/filters.json`，对其它浏览器/访客的最终一致性不变。验证：Playwright 让 `/api/live` 返回**仍含被删 `infra` 的陈旧 `live/filters.json`**（复现保存失败/R2 延迟场景），页面脚本运行前用 `addInitScript` 预置 tombstone，刷新后——修复前公开页与管理页都 `deletedFilterPresent:true`（FAIL），修复后均 `false`（PASS）；健全性：无 tombstone 时 7 个 filter 全部正常渲染。`pnpm build` 34 页通过、`check-pre-whitespace` 通过。红线：未改动高危文件（BaseLayout/index/global.css 未动）。

42. **修复 filter 在管理页点编辑后重新出现（第三轮，`mergedFilters()` 逻辑修正，覆盖条目 41 的残留）**：用户反馈「filter 在预览页面删除了，在管理页面点右下角编辑的时候还在，退出编辑后又不在了」。重读整个 filter 命令流后定位：问题在 `mergedFilters()`。删除 filter 时 `deleteFilter()` 只向 `localStorage` `push` 一条 tombstone，不清除同一 slug 此前由「新建 Filter」或「编辑图片」写入的非 tombstone 本地记录；进入编辑态时 `setupCouplingPageEdit()` → `rebuildStripFromMerged()` → `mergedFilters()` 先把静态 filter 中被 tombstone 的 slug 剔除，但遍历本地记录时**只跳过 `images === "__DELETED__"` 的条目**，导致同一 slug 的非 tombstone 本地记录因 `findIndex === -1` 被重新 `push` 回结果——编辑态下被删 filter 复现。退出编辑态后用户看到它“不在”，是因为非编辑态由 `live-patch.ts` 渲染（条目 41 已跳过 tombstone），而退出不重绘 strip，视觉上回到非编辑态。修复两处：
    - `mergedFilters()`：本地记录循环改为 `if (deletedSlugs.has(lf.slug)) continue;`——只要 slug 被 tombstone，任何本地记录（新增/编辑）都忽略。
    - `deleteFilter()`：在 push tombstone 之前先用 `.filter((f) => f.slug !== slug)` 清除该 slug 的所有历史本地记录，防止累积；同时避免「删除后又重建同名 filter」时旧 tombstone 把新 filter 也跳过。
    - `createNewFilter()`：push 新 filter 前也清除同一 slug 的本地记录（含 tombstone），保证重建同名 filter 能真正生效。
    验证：Playwright 预置同一 slug 的「旧编辑记录 + tombstone」两种本地记录，并 mock `/api/live` 返回仍含该 slug 的陈旧 `live/filters.json`——修复前管理页非编辑态隐藏、进入编辑态复现（`present:true, count:7`），退出后仍复现（FAIL）；修复后公开页非编辑、管理页非编辑、管理页编辑态、退出后四个状态均隐藏（`present:false, count:6`，PASS）。同时跑通条目 41 的 stale-live 回归测试。`pnpm build` 34 页通过、`check-pre-whitespace` 通过。红线：未改动高危文件。

43. **filter strip 末尾加英文句号（替代逗号）**：用户要求 filter 词组最后一个用「. 」而非「, 」，且以后新增的也遵循。改动：
    - `src/components/CouplingFilterStrip.astro`：服务端渲染时 `index < length - 1 ? ", " : ". "`。
    - `public/filter-edit.js`：新增 `refreshFilterSeparators()`——遍历 `.filter-strip-item`，除最后一项的 `.filter-comma` 为 `. ` 外其余为 `, `；在 `appendFilterToStrip()`（新增/动态追加，含 `createNewFilter`）、`deleteFilter()`（删除后新尾部变句号）、`rebuildStripFromMerged()`（进编辑态重绘）、`applyTombstonesOnInit()`（删除 tombstone 后）四处调用；并清理了 `appendFilterToStrip` 里遗留的无用 `.filter-sep-last` 处理。同时把 `deleteFilter()` 的 DOM 移除移到 `saveFiltersToServer` 之前，删除后即时从 strip 移除并刷新分隔符，响应更直接（本地 tombstone 仍保证刷新后隐藏）。
    - `src/scripts/live-patch.ts`：filter 重注入时按 `visibleFilters` 索引设分隔符，最后一项 `. `、其余 `, `。
    验证：Playwright 覆盖——公开静态 / 公开 live-patch / 管理非编辑 / 管理编辑态 四种状态的 strip 尾部均为 `. `、其余 `, `；删除最后一个 filter 后新尾部 `11` 变为 `. `；`appendFilterToStrip` 追加新 filter 后新尾部 `new-one` 变为 `. `；连同条目 40/41/42 的删除回归测试一并通过。`pnpm build` 34 页通过、`check-pre-whitespace` 通过。红线：未改动高危文件（BaseLayout/index/global.css 未动）。

44. **修复新建 filter 后预览页不显示**：用户反馈管理页点编辑新建 filter 后，预览页看不到。重读 filter 全链路：`createNewFilter()` 写入 localStorage 并调用 `saveFiltersToServer(mergedFilters())`；服务器 `save-filters` 会把合并后的 filter 列表同时写入 GitHub `couplingFilters.ts` 与 R2 `live/filters.json`；公开预览页 `/coupling/` 的静态 strip 来自旧的 `couplingFilters.ts`（重建前无新 filter），由 `live-patch.ts` 取 `/api/live`（即 `live/filters.json`）重注入。代码逻辑正确，但生产环境若 R2 `live/filters.json` 写入失败/延迟（如 `EDITOR_BUCKET` 绑定缺失、R2 跨区域传播滞后），预览页就无法即时看到新 filter。修复：在 `live-patch.ts` 中把 server filters 与 `localStorage["am_filter_edit_v1"]` 中本地新增的 filter 合并——服务器数据优先用于已存在 slug，本地独有的 slug（新建未同步）补进列表；同时仍按 tombstone 排除已删 slug。这样即使 `live/filters.json` 为空/陈旧，只要在同一浏览器新建过 filter，预览页也能立即显示；R2/GitHub 同步完成后自动切换到服务器数据。验证：Playwright 模拟两种场景——A) `/api/live` 返回含 new-filter 的更新列表，预览页显示 8 个；B) `/api/live` 返回不含 new-filter 的陈旧列表，但同一浏览器 localStorage 中有 new-filter，预览页仍显示 new-filter（PASS）。同时跑通删除、编辑态、分隔符三套回归测试。`pnpm build` 34 页通过、`check-pre-whitespace` 通过。红线：未改动高危文件。

45. **加固 filter tombstone：即使 `/api/live` 无数据也隐藏已删 filter**：在条目 41/44 基础上重读 `live-patch.ts`，发现一个仍会触发「删除的 filter 刷新后又出现」的缺口：`patchLive()` 早退 `if (!map || Object.keys(map).length === 0) return;` 在 `/api/live` 返回 `{}`（如 R2 `EDITOR_BUCKET` 绑定缺失、live 数据完全不可用时）会整体跳过，于是 `am_filter_edit_v1` 里的删除 tombstone 永远不会在预览页生效，已删 filter 仍显示。修复：把 `getLocalFilterEntries()`/`getDeletedFilterSlugs()` 上提到早退判断之前，仅当「map 为空 且 本地无任何 filter 改动」时才早退；filter 注入块的触发条件由 `Array.isArray(map.filters) || 本地有新增` 扩展为 `serverFilters || 本地新增 || 本地删除`，并对「live filters 缺失/为空」的情况改为以 DOM 中已服务端渲染的静态 strip 为基准（不再清空），在其上叠加本地删除与新增。验证：新增第 5 套 Playwright 回归 `reproduce_delete_no_live.mjs`，模拟 `/api/live` 返回 `{}` 与 `{filters:[]}` 两种「R2 不可用」形态——已删 slug 在预览页均被隐藏、其余 filter 保留（PASS）；并连同前 4 套（删除刷新、编辑态一致、末尾句号、新建显示）共 5 套全部 PASS。`pnpm build` 34 页通过、`check-pre-whitespace` 通过。红线：仅改 `live-patch.ts`，未碰高危文件。
...
- 最新提交（main HEAD）：`(filter tombstone harden)`（fix(filter): apply deletion tombstones even when /api/live returns no filters）。本次新增 ⑲ 加固 filter tombstone——`live-patch.ts` 早退判断会整体跳过「`/api/live` 返回 `{}`」的情形，导致 R2 不可用时已删 filter 在预览页仍显示；已把本地 filter 改动判断上提、并改为以 DOM 静态 strip 为基准叠加删除/新增（详见 §4 条目 45）。此前的 ⑱ 修复新建 filter 后预览页不显示——`live-patch.ts` 原本只从 `live/filters.json` 重注入 filter，若 R2 写入失败/延迟则新建 filter 无法在预览页即时出现；已改为把 server filters 与 `localStorage["am_filter_edit_v1"]` 中本地新增的 filter 合并（服务器数据优先、本地独有 slug 补齐、tombstone 排除已删 slug），使同一浏览器内新建的 filter 即使 `live/filters.json` 为空/陈旧也能在预览页显示（详见 §4 条目 44）。此前的 ⑰ 修复 filter 末尾加英文句号（替代逗号）——已统一服务端渲染、动态追加、live 重注入三处分隔符逻辑（详见 §4 条目 43）。此前的 ⑯ 修复 filter 在管理页点编辑后重新出现（`mergedFilters()` 逻辑修正，覆盖条目 41 残留）——已改为按 slug 跳过所有被 tombstone 的本地记录，并在 `deleteFilter()`/`createNewFilter()` 中清理同一 slug 的历史本地记录（详见 §4 条目 42）。此前的 ⑮ 修复 filter 删除后刷新又出现（`live-patch.ts` 兜底，覆盖条目 40 残留）——已在 `live-patch.ts` 读取 `am_filter_edit_v1` 删除 tombstone 并重注入时跳过被删 slug（详见 §4 条目 41）。此前的 ⑭ 修复 filter strip 点 × 删除 filter 后刷新又出现（`filter-edit.js` 侧）——已在 init 时立即应用 tombstone，把静态 strip 中对应项移除（详见 §4 条目 40）。此前的 ⑬ overlay 详情层「添加/删除图纸」迁到底部左侧控制条——`ProjectDetailOverlays.astro` 增加 `overlay-edit-strip` 与 `data-role="delete-gallery"` 删除按钮；`editor.css` 在 `html.detail-overlay-open body.edit-mode` 下把该条以 `position: sticky` 吸在 overlay 底左（内容短时自然落在画廊底部，内容长时随滚动吸底，避免被裁出可视区）；`edit-mode.ts` 新增删除处理并把逐张图 X 的删除逻辑抽成 `removeGalleryWrap()`，同时修复 detach 后 `closest(".project-card")` 为 null 导致 frontmatter 不更新的隐患。此前的 ⑫ 修复「在详情页改封面图、预览页项目图片不更新」——全站「换封面图」控件只在项目详情页 `article.project-detail.project-card`，但 `save()` 原不收集它（封面既不写回 GitHub frontmatter、也不写 `live/*.json` 即时覆盖），且 `live-patch.ts` 取 slug 时误命中详情页内部「配对项目」`ProjectCard` 的嵌套 `view-link` 导致 hero 不替换；已把 `save()` 选择器扩到 `article.project-detail.project-card`、并对 `project-detail` 卡片直接读 `card.dataset.slug`（详见 §4 条目 38）。此前的 ⑪ 修复 overlay 详情页添加/删除/拖拽图纸后保存报 GitHub `tree 422`——`save()` 只收集 `.project-column .project-card`（overlay 里的 `.project-card` 被排除以避免重复路径），但画廊操作的 `markDirty()` 只标记了 overlay 卡片自己，导致列卡片未 dirty、空 items 触发 422；新增 `persistCardChange()` 把 overlay 的最新 frontmatter 同步到同 slug 列卡片并标记 dirty，后端同时防御空 items 直接返回 ok（详见 §4 条目 37）。本会话此前修复：⑩ 管理页 filter strip 删除按钮 × 位置修正——由「每个词组最左边（实际在下一个词组左侧）」修正为紧邻自身 Label 右侧，静态 `CouplingFilterStrip.astro` 与动态 `appendFilterToStrip()` 词条顺序均改为 `Label → × → , `（详见 §4 条目 36）；⑨ 管理页顶部 filter strip 链接——点击 `/editor/coupling/` 或 `/editor/filter/[slug]/` 上的 filter 词组现在进入 `/editor/filter/[slug]/` 管理编辑页，不再进入 `/filter/[slug]/` 预览页（commit `594a221`，详见 §4 条目 35）；⑧ 公开 `/filter/[slug]/` 词条页最右侧图片被纵向裁切修复（commit `b77740a`，详见 §4 条目 34）；⑦ round-3：filter 词条页（`/editor/filter/[slug]/`）同样把左下角绿色返回链接与「编辑此 Filter / + 添加图片 / 保存更改 / 退出」工具栏收口到右侧「编辑」栏（`+ 添加图片` / `保存更改 (N 删除)`），底部仅留可编辑 INFO；⑥ 修复新建 Filter 时 `保存失败： unauthorized`（密码 `trim()` + 401 重输）；⑤ round-2：耦合管理页底部左侧彻底清空，filter 编辑入口移入右侧「编辑」栏（`新建 Filter` 多选建 Filter，底部工具栏强制隐藏）；④ 耦合管理页底部对齐首页（可编辑 INFO，工具栏默认隐藏）；③ 主页 `/editor` 的 COUPLING 链接改指 `/editor/coupling/`，管理套件返回链串接；② 重写 `filter-edit.js` 为合法纯 JS；① coupling/filter 预览与管理 URL 分离。均已 push 至 `origin/main`，Cloudflare 自动部署中。历史：info 相关 `b077e5f` + `61efd19` 已部署；`65513e8` 为初版 filter 编辑器（其 `filter-edit.js` 当时即已损坏，本会话修复）。

...
- 最新提交（main HEAD）：合并 `b77740a`（公开词条页裁切修复）与 `594a221`（管理页 strip 链接指向 /editor/filter/）；本次（Issue 3 follow-up）新增 ⑩ 管理页 filter strip 删除按钮 × 位置修正——由「每个词组最左边（实际在下一个词组左侧）」修正为紧邻自身 Label 右侧，静态 `CouplingFilterStrip.astro` 与动态 `appendFilterToStrip()` 词条顺序均改为 `Label → × → , `（详见 §4 条目 36）。本会话（裁切修复）、同线另一会话（strip 链接修复）及本次 × 修复均已 push 至 `origin/main`，Cloudflare 自动部署中。近期修复：① coupling/filter 预览与管理 URL 分离；② 重写 `filter-edit.js` 为合法纯 JS；③ 主页 `/editor` 的 COUPLING 链接改指 `/editor/coupling/`，管理套件返回链串接；④ 耦合管理页底部对齐首页（可编辑 INFO，工具栏默认隐藏）；⑤ round-2：耦合管理页底部左侧彻底清空，filter 编辑入口移入右侧「编辑」栏（`新建 Filter` 多选建 Filter，底部工具栏强制隐藏）；⑥ 修复新建 Filter 时 `保存失败： unauthorized`（密码 `trim()` + 401 重输）；⑦ round-3：filter 词条页（`/editor/filter/[slug]/`）同样把左下角绿色返回链接与「编辑此 Filter / + 添加图片 / 保存更改 / 退出」工具栏收口到右侧「编辑」栏（`+ 添加图片` / `保存更改 (N 删除)`），底部仅留可编辑 INFO；⑧ 公开 `/filter/[slug]/` 词条页最右侧图片被纵向裁切修复——根因误用 `.filter-board` 横向 flex masonry 超出视口、被 `main{overflow-x:hidden}` 裁掉，改为默认 `.coupling-board` 纵向网格 masonry（与主页、管理页一致），仅 `src/pages/filter/[term].astro` 删 `boardClass` 一处（commit `b77740a`，详见 §4 条目 34）；⑨ 管理页顶部 filter strip 链接——点击 `/editor/coupling/` 或 `/editor/filter/[slug]/` 上的 filter 词组现在进入 `/editor/filter/[slug]/` 管理编辑页，不再进入 `/filter/[slug]/` 预览页（commit `594a221`，详见 §4 条目 35）。历史：info 相关 `b077e5f` + `61efd19` 已部署；`65513e8` 为初版 filter 编辑器（其 `filter-edit.js` 当时即已损坏，本会话修复）。
